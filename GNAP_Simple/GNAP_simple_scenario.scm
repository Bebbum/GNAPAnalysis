...
(defprotocol single_token_simple basic
	(defrole client
		...
		(trace
			...
			; or not with the AS.
			(send (enc c access (ltk c as))) ; client requests access at the AS by identifying itself and proving possession of a cryptogrphic key
			(recv (enc (enc (cat acess_token value access_type (pubk as)) (pubk rs)) (ltk as c))) ; receives and decrypts message using private key whic results in a usable access token
			(send (enc (enc (cat acess_token value access_type (pubk as)) (pubk rs)) (ltk c rs))) ; The client instance uses the access token to call the RS.
	...
	(defrole authorization_server
		...
		(trace
			(recv (enc c access (ltk c as))) ; AS validates the client's request
			...
			(send (enc (enc (cat acess_token value access_type (pubk as)) (pubk rs)) (ltk as c))) ; AS signs token and binds it to client's public key
		)
	)
	(defrole resource_server
		(vars (c as rs name) (acess_token value access_type response data))
		(trace
			(recv (enc (enc (cat acess_token value access_type (pubk as)) (pubk rs)) (ltk c rs))) ; RS decrypts and validates the client's token
