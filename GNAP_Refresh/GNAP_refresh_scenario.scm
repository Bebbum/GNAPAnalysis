...
(defprotocol token_refresh basic
	(defrole client
		(vars (c as rs name) (access management_uri management_uri_new response response_error data) (n1 n2 n3 n4 token token_new text))
		(trace
			...
			(recv (enc management_uri_new token (hash n1 n2)))
	...
	(defrole authorization_server
		...
		(trace
			...
			(send (enc as n1 n2 (pubk c))) (recv (enc n2 (pubk as))) (recv (enc c access (hash n1 n2)))
	...
	(defrole resource_server
		(vars (c as rs name) (access_token access_token_new value access_type response response_error data) (n3 n4 token req1 req2 text))
		(trace
			(recv (enc c n3 (pubk rs)))
			(send (enc rs n3 n4 (pubk c)))
			(recv (enc n4 (pubk rs)))
			(recv (enc req1 (enc (enc  (cat access_token value access_type) (privk as)) (pubk rs)) (hash n3 n4)))
